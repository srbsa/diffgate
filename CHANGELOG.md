...
**Bug-bash round three** — a feature audit of the 0.8.0 surface plus an adversarial security pass over all three entry points (CLI, MCP, VS Code). Every fix below is pinned by a regression test in [test/bugbash-round-three.test.js](test/bugbash-round-three.test.js); each one shipped *green*, because no test exercised the path.

*Security (all three surfaces):*

- **A repo-tracked `.diffgate.json` could exfiltrate any environment variable to any host.** `ai.baseURL` and `ai.apiKeyEnv` were trusted verbatim from repo config, and `complete()` sent `process.env[apiKeyEnv]` as a bearer token to `baseURL` — so a hostile repo could name *any* variable (`AWS_SECRET_ACCESS_KEY`, not just an LLM key) and any destination, and an agent calling `diffgate_explain` over MCP (or a user clicking "Explain" in the editor) would ship it. Same shape as CVE-2026-21852 in Claude Code, and fixed the same way — gate the fields that decide where a credential goes behind something the repo cannot write. `apiKeyEnv` from config is now honored only when it names a known provider key env; `baseURL` only when it is loopback (the shared local-model case). Non-local custom endpoints come from `$DIFFGATE_AI_BASE_URL` / `$DIFFGATE_AI_API_KEY_ENV`. `isAiAvailable` reports false and `complete()` refuses to send rather than falling back to a default endpoint.
- **One malformed stdio line killed the MCP server.** `dispatchMessage` destructured the message before any guard, so a bare `null` (valid JSON, invalid message) threw a `TypeError` out of a `void`-ed async call with no `.catch` — the whole process exited 1, taking the agent's tool connection with it. Non-object messages are now dropped.
- **`diffgate_analyze` was an arbitrary file read.** `filePath` accepted any absolute or `../`-relative path with no containment check, and any secret-shaped line came back verbatim in `finding.code` — a read-only-annotated tool that could exfiltrate `/etc/passwd` or a `.env` outside the repo. Paths are now resolved (symlinks included, on both sides) and must land inside the repo `cwd`.
- **`install-hook` corrupted its own merge-driver registration on any path containing a single quote.** `execSync` with a hand-quoted shell string broke out on e.g. `/Users/o'brien/…`; the `git config` call failed into a bare `catch`, but `.gitattributes` had already been written — leaving every future merge pointing at a driver that was never registered. Now `execFileSync` with an argv array (no shell).
- **`diffgate watch` could die on a single bad file.** The chokidar handler guarded `readFileSync` but not `analyze()`, and the async callback runs outside `main()`'s try/catch. Now guarded, and reports the file instead of exiting.
- **The VS Code extension declared no workspace-trust posture**, so it ran at full strength — repo config included — in a Restricted-Mode window. Now declares `untrustedWorkspaces: limited` with the `diffgate.ai.*` settings restricted.

*Correctness:*

- **A user's `tier`/`blocking` override was silently discarded by a rule's own per-finding adjustment.** `makeFinding` preferred the emit-site tier unconditionally, so `rules: { "path-traversal": { tier: "orange" } }` still reported yellow whenever the rule's sanitizer heuristic fired. An override now pins the tier (and suppresses the `tierAdjusted` label, which claimed a change that no longer happens).
- **Python `match` and PHP `match` scored double, not zero.** Both were missing from their profiles entirely (a `match` block scored 0 cognitive complexity and opened no nesting level). Adding them surfaced a second, older defect: tree-sitter names every `case_clause`/match arm an `"alternative"` field, and `elseBranches` treated the first arm as a flat `else`-chain link — so once scored, a `match` charged 1 for the construct *plus* 1 for its first arm, where the equivalent `if` charged 1. Switch-like constructs and their arm types are now excluded from else-chain detection (guarded on both the parent and the child type, since Python re-exposes the field on the wrapping `block`). A `match` now scores exactly like `switch`, and if/elif/else chains are unaffected.
- **C# was structurally invisible to `pass-through-wrapper`** for two independent reasons: `CALL_NODE_TYPES` listed `call_expression` (the grammar uses `invocation_expression`), and C# wraps each call argument in its own `argument` node that the forwarding check never unwrapped. Neither was caught because the structural suite contained no C# case at all.
- **C# constructors and local functions were invisible to the complexity engine and the reinvention index** — `functionTypes` was `method_declaration` only.
- **Java entry-point detection had never worked.** `method.childForFieldName("modifiers")` is not a field in tree-sitter-java (annotations are unordered children), so every method was skipped and the detector matched nothing — Spring included, not just the missing JAX-RS annotations it was reported for. Fixed, and `@GET`/`@POST`/`@PUT`/`@DELETE`/`@PATCH` added for Jersey/RESTEasy/Quarkus.
- **Go entry-point detection missed TitleCase router verbs** (`app.Get`, `r.Post`) used by Fiber, chi, and gorilla/mux. Added, with a string-literal route requirement so everyday calls like `cache.Get(key, ok)` don't register as routes.
- **`ts-over-generic` counted any node whose type name contained `"Type"`**, so array, tuple, union, intersection, parenthesized, and function types each added a phantom level: `string[][][][]` and `(A|B|C|D)[]` scored 4 with zero generics, and a curried function type scored 5. Only a type reference carrying its own type arguments counts now; everything else is a transparent wrapper.
- **`diff-churn-ratio` had no tree-sitter implementation**, so it covered JS/TS only while the entry above claimed all 8 languages. `DiffChurnRatioTsAst` closes it.
- **`pass-through-wrapper` and `reinvented-helper` skipped class and object methods** (`ClassMethod`, `ObjectMethod`, `ClassPrivateMethod`) even though `extractBabelName` and the fingerprint index already handled them — a class method could be the thing you duplicated but never the thing that got flagged.
- **`extractBabelName` missed member-expression bindings and private names.** `this.foo = () => {}`, `obj.foo = () => {}`, and `#foo() {}` all resolved to no name and were dropped, despite having call sites the "inline it" advice applies to.
- **Entry-point dedup dropped a second inline handler registered on the same call** (`app.get("/p", mw, handler)` where both start on one line); the key now includes the argument index. Handlers passed inside an array argument (`app.get("/p", [mw1, mw2], handler)`) were ignored entirely and are now walked.
- **`history.ts` carried a seventh hand-rolled copy of the recompute-result block** that `recomputeResult` was introduced to eliminate — missed by `reinvented-helper` itself because the copy is an inline expression in a loop body, not a function with a shape to fingerprint.
- **Dead threshold config removed** — `maxClassLines` for Go (which has no classes) and the whole Rust/C++/C block (no grammar is loaded for them, so nothing could ever read it).
