...
- **Real AST for JS/TS and Python:** `@babel/parser` (JS/TS) and tree-sitter (Python, via WASM — no native build) power precise rules: deprecated calls aren't matched inside comments or strings; exported-signature changes are detected structurally; Python SQL injection is **sink-targeted, parameter-aware, and sanitizer-aware** — `cur.execute(f"… {uid}")` blocks, `cur.execute("… %s", (uid,))` and a `SELECT` in a log line don't.
- **Comment-aware pattern rules:** secrets, SQL/schema changes, auth/crypto, dynamic execution, and raw queries across Go, Java, Ruby, PHP, and any text. Commented-out code (`# os.system(x)`) isn't flagged; a secret committed *inside* a comment still is. Injection-sink rules are AST-deep on JS/TS and Python and best-effort regex on the rest — see **[docs/SCOPE.md](docs/SCOPE.md)** for the per-language coverage tiers.
- **Non-JS/Python injection that earns its block:** the broad cross-language injection advisories (PHP/Ruby interpolation, Go/Ruby shell-out) stay advisory on their own and **escalate to a blocking finding only when the optional code graph proves the sink is reachable from an untrusted entry point** (an HTTP/event handler) — community CodeGraph, no Pro taint engine. Recall from the rules, the right to block from the graph. (The AST layer and the graph compose: AST gives intra-function precision, the graph adds cross-file reachability and blast radius.)
