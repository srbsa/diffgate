...
- **Earned blocking:** broad cross-language injection advisories (Python/Ruby idioms, Go/Ruby shell-out) escalate to blocking **only when the optional code graph proves reachability from an untrusted entry point** — community CodeGraph, no Pro taint engine required.
...
| **Deep (AST)** | Python | AST-precise **SQL injection**, **XSS** (`mark_safe`/templating), **path traversal** (request-source + sanitizer-aware), and **permissive CORS** (flask-cors / django-cors-headers). |
| **Deep (AST)** | PHP | AST-precise **SQL injection** (sink/parameter/sanitizer-aware). Other footgun classes are **not yet** AST rules here. |
| **Floor (pattern)** | Go, Java, Ruby, C/C++, C#, Rust, … | Secrets, destructive/schema changes, auth/crypto, dynamic exec / shell-out, raw queries, network calls, TODO. Cross-language injection advisories that escalate via the code graph. |
