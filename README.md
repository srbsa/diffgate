...
- **Real AST for JS/TS, Python, and PHP:** `@babel/parser` (JS/TS) and tree-sitter (Python + PHP, via WASM — no native build) power precise rules: deprecated calls aren't matched inside comments or strings; exported-signature changes are detected structurally; SQL injection is **sink-targeted, parameter-aware, and sanitizer-aware** — `cur.execute(f"… {uid}")` / `$pdo->query("… $id")` block, while `cur.execute("… %s", (uid,))`, `$pdo->prepare("… ?")`, a single-quoted `'… $id'`, and a `SELECT` in a log line don't.
- **Comment-aware pattern rules:** secrets, SQL/schema changes, auth/crypto, dynamic execution, and raw queries across Go, Java, Ruby, and any text. Commented-out code (`# os.system(x)`) isn't flagged; a secret committed *inside* a comment still is. Injection-sink rules are AST-deep on JS/TS, Python, and PHP and best-effort regex on the rest — see **[docs/SCOPE.md](docs/SCOPE.md)** for the per-language coverage tiers.
- **Other-language injection that earns its block:** the broad cross-language injection advisories (Ruby interpolation, Go/Ruby shell-out) stay advisory on their own and **escalate to a blocking finding only when the optional code graph proves the sink is reachable from an untrusted entry point** (an HTTP/event handler) — community CodeGraph, no Pro taint engine. Recall from the rules, the right to block from the graph. (The AST layer and the graph compose: AST gives intra-function precision, the graph adds cross-file reachability and blast radius.)
